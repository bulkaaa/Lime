...
public class SmoothingService implements ISmoothingService{
    ...
    private static TimeSeries DoubleExpSmoothing(final TimeSeries ts, final double alpha, final double beta,
                                                 final int days) {
    ...
    private static TimeSeries TripleExpSmoothing(final TimeSeries ts, final double alpha, final double beta,
                                                 final double gamma, final int days) {
        final TimeSeries SmoothTS = new TimeSeries();
        final TimeSeries TrendTS = new TimeSeries();
        ...
        for (int i = 0; i < days; i++) {
            SmoothTS.add(
                    (SmoothTS.get(ts.size() - 1) + (i + 1) * TrendTS.get(ts.size() - 1)) * SeasonTS.get(i - 7 + i + 1));
        }
        return SmoothTS;
    }
}