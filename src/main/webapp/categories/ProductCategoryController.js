app.controller('ProductCategoryController', ['$scope', '$rootScope', '$http', '$uibModal', 'dialogs', 'DialogService', function($scope, $rootScope, $http, $modal, $dialogs, DialogService) {
    var modalInstance = null;
    $scope.categoryProduct = true;
    ...
    $scope.editRecord = function(item){
        $http.get("/product-category/one/" + item.id)
    ...
    $scope.updateRecord = function(item) {
        $http.put("/product-category/update", JSON.stringify(item))
            .then(
                function(response){
                    if (response.data){
                        item = response.data;
    ...
    $scope.saveRecord = function(item) {
        ...
        $http.post("/product-category/create", JSON.stringify(item))
            .then(
                function(response){
                    if (response.data){
