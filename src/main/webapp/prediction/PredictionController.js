...
$scope.getProducts = function() {
    ...
    $scope.generateForecast = function () {
        ...
        $http.post("/forecast/generate" + "?email=" + $scope.email + "&startDate=" + date.getDate()+"-"+date.getMonth()+1+"-"+date.getFullYear() + "&noDays=" + $scope.noDays + "&noDaysForecast=" + $scope.noDaysForecast, JSON.stringify(item))
            .then(
                function(response){
                    if (response.data){
                    }
