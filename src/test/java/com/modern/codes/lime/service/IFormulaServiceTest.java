...
public class IFormulaServiceTest extends IBasicCRUDServiceTest<Formula, FormulaPOJO, IFormulaDAO, FormulaService> {
    ...
    public void updateTest() {
        final FormulaPOJO obj = service.findAll()
                                       .get(0);
        assertNotEquals("0test", obj.getValue());
        obj.setValue(10101010.00);
        service.save(obj);
        assertEquals(new Double(10101010.00), service.findById(obj.getId())
                                                     .getValue());
    }
    ...
    public void updateByListTest() {
        ...
        assertNotEquals("1test", list.get(1)
                                     .getValue());
        list.get(0)
            .setValue(10101010.00);
        list.get(1)
            .setValue(10101011.00);
        service.save(list);
        assertEquals(new Double(10101010.00), service.findById(list.get(0)
                                                                   .getId())
                                                     .getValue());
        assertEquals(new Double(10101011.00), service.findById(list.get(1)
                                                                   .getId())
                                                     .getValue());
    }
    @Autowired
